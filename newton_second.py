import numpy as np
from mpl_toolkits.mplot3d import Axes3D
import matplotlib.pyplot as plt
import matplotlib
matplotlib.rcParams['legend.fontsize'] = 20

from laws import newton
from machine import Machine

m = np.arange(7.0, 8.0, 0.02)
a = np.arange(5.0, 6.0, 0.02)
X_test, Y_test = np.meshgrid(m, a)
m_a = np.array(list(zip(np.ravel(X_test), np.ravel(Y_test))))

xs = np.arange(7.0, 8.0, 0.1)
ys = np.arange(5.0, 6.0, 0.1)
X = np.array([(x, y) for x in xs for y in ys])

Mdm = Machine(model='dm')

dmm, dma = Mdm.d(X)

Mdmm = Machine((2), nodes = 128, lr = 0.01)
Mdmm.learn(X, dmm, 4096)
Mdmm.save("dmm")

zs = Mdmm.v(m_a)
Z = zs.reshape(X_test.shape)

zs_t = np.array([0.0] * (len(X_test) * len(X_test[0])))
Z_t = zs_t.reshape(X_test.shape)

# plot
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
surf = ax.plot_surface(X_test, Y_test, Z, label = "prediction")
surf._facecolors2d=surf._facecolors3d
surf._edgecolors2d=surf._edgecolors3d
surf = ax.plot_surface(X_test, Y_test, Z_t, label = "actrual")
surf._facecolors2d=surf._facecolors3d
surf._edgecolors2d=surf._edgecolors3d
ax.set_title("ddF/dmdm")
ax.set_xlabel('m')
ax.set_ylabel('a')
ax.set_zlabel('F_mm')
ax.legend()

Mdma = Machine((2), nodes = 128, lr = 0.01)
Mdma.learn(X, dma, 4096)
Mdma.save("dma")

zs = Mdma.v(m_a)
Z = zs.reshape(X_test.shape)

zs_t = np.array([1.0] * (len(X_test) * len(X_test[0])))
Z_t = zs_t.reshape(X_test.shape)

# plot
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
surf = ax.plot_surface(X_test, Y_test, Z, label = "prediction")
surf._facecolors2d=surf._facecolors3d
surf._edgecolors2d=surf._edgecolors3d
surf = ax.plot_surface(X_test, Y_test, Z_t, label = "actrual")
surf._facecolors2d=surf._facecolors3d
surf._edgecolors2d=surf._edgecolors3d
ax.set_title("ddF/dmda")
ax.set_xlabel('m')
ax.set_ylabel('a')
ax.set_zlabel('F_ma')
ax.legend()

Mda = Machine(model='da')

dam, daa = Mda.d(X)

Mdam = Machine((2), nodes = 128, lr = 0.001)
Mdam.learn(X, dam, 4096)
Mdam.save("dam")

zs = Mdam.v(m_a)
Z = zs.reshape(X_test.shape)

zs_t = np.array([1.0] * (len(X_test) * len(X_test[0])))
Z_t = zs_t.reshape(X_test.shape)

# plot
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
surf = ax.plot_surface(X_test, Y_test, Z, label = "prediction")
surf._facecolors2d=surf._facecolors3d
surf._edgecolors2d=surf._edgecolors3d
surf = ax.plot_surface(X_test, Y_test, Z_t, label = "actrual")
surf._facecolors2d=surf._facecolors3d
surf._edgecolors2d=surf._edgecolors3d
ax.set_title("ddF/dadm")
ax.set_xlabel('m')
ax.set_ylabel('a')
ax.set_zlabel('F_am')
ax.legend()

Mdaa = Machine((2), nodes = 128, lr = 0.001)
Mdaa.learn(X, daa, 4096)
Mdaa.save("daa")

zs = Mdaa.v(m_a)
Z = zs.reshape(X_test.shape)

zs_t = np.array([0.0] * (len(X_test) * len(X_test[0])))
Z_t = zs_t.reshape(X_test.shape)

# plot
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
surf = ax.plot_surface(X_test, Y_test, Z, label = "prediction")
surf._facecolors2d=surf._facecolors3d
surf._edgecolors2d=surf._edgecolors3d
surf = ax.plot_surface(X_test, Y_test, Z_t, label = "actrual")
surf._facecolors2d=surf._facecolors3d
surf._edgecolors2d=surf._edgecolors3d
ax.set_title("ddF/dada")
ax.set_xlabel('m')
ax.set_ylabel('a')
ax.set_zlabel('F_aa')
ax.legend()

plt.show()

M = Machine(model = "ma")
Mdm = Machine(model = "dm")
Mda = Machine(model = "da")

X0 = np.array([[7.0, 5.0], [7.0, 6.0], [8.0, 5.0], [8.0, 6.0], [7.5, 5.5]])

dm, da = M.d(X0)
dmm, dma = Mdm.d(X0)
dam, daa = Mda.d(X0)

print(M.v(X0), dm, da, dmm, dma, dam, daa, sep = '\n')
